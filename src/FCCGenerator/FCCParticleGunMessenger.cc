...
//
//
// $Id: FCCParticleGunMessenger.cc 79124 2014-02-18 15:04:32Z gcosmo $
//
...
FCCParticleGunMessenger::FCCParticleGunMessenger(FCCParticleGun * fPtclGun)
   ...
   fAtomicNumber(0),fAtomicMass(0),fIonCharge(0),fIonExciteEnergy(0.0),fIonEnergyLevel(0)
{
  ...
  // set initial value to FCCParticleGun
...
FCCParticleGunMessenger::~FCCParticleGunMessenger()
{
  delete listCmd;
  delete particleCmd;
  delete directionCmd;
...
void FCCParticleGunMessenger::SetNewValue(G4UIcommand * command,G4String newValues)
{
  if (command==listCmd) {
...
G4String FCCParticleGunMessenger::GetCurrentValue(G4UIcommand * command)
{
  G4String cv;
  ...
  else if( command==energyCmd )
  {
    ...
    if(ene == 0.)
    { G4cerr << " FCCParticleGun:  was defined in terms of momentum." << G4endl; }
    else
  ...
  else if( command==momCmd || command==momAmpCmd )
  {
    ...
    if(mom == 0.)
    { G4cerr << " FCCParticleGun:  was defined in terms of kinetic energy." << G4endl; }
    else
    {
      if( command==momCmd )
...
void FCCParticleGunMessenger::IonLevelCommand(G4String newValues)
{
  if (fShootIon) {
...
void FCCParticleGunMessenger::IonCommand(G4String newValues)
{
  if (fShootIon) {
    G4Tokenizer next( newValues );
    // check argument
